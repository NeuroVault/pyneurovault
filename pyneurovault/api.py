...
from pyneurovault.utils import get_json_df, mkdir_p, split_filename
...
def collections_from_dois(dois, limit=1000):
...
def get_data(data_type, pks=None, params=None, extend_url=None):
    ...
    if not pks:
        ...
        data = get_json_df(
            data_type=data_type, pks=pks, params=params, extend_url=extend_url
        )
...
def get_images(pks=None, collection_pks=None, limit=1000, params={}):
    ...
    if "collection" in images.columns:
        images["collection_id"] = images["collection"].apply(
...
def get_collections(pks=None, limit=1000, params={}):
    """
        ...
        if not os.path.exists(orig_file):
            try:
                ...
                if resample == True:
                    ...
                    if (
                        np.all(np.isnan(out_of_mask))
                        or len(np.unique(out_of_mask)) == 1
                    ):
                        # Need to extrapolate
                        data = _extrapolate_out_mask(
                            data.astype(np.float), bg_mask, iterations=3
                        )[0]
                    niimg = nb.Nifti1Image(data, affine, header=niimg.get_header())
                    ...
                    if len(resampled_nii.shape) == 3:
                        ...
                        for index in range(resampled_nii.shape[-1]):
                            ...
                            this_file = os.path.join(
                                resampled_path, "%06d%s" % (this_id, ext)
                            )
                ...
                print(
                    "Error downloading image id %s, retry this image."
                    % (row[1]["image_id"])
                )
    return out_df
