...
class Pubmed:
        ...
        if "IdList" in record:
            if record["Count"] != "0":
                ...
                handle = Entrez.efetch(
                    db="pubmed", id=record["IdList"][0], retmode="xml", retmax=1
                )
                record = Entrez.read(handle)
                record = record[0]
    ...
    def search_article_list(self, article, term):
        ...
        [
            [words.append(str(porter.stem(t))) for t in word_tokenize(x.lower())]
            for x in term
        ]
    ...
    def get_author_articles(self, author):
                                ...
                                for r in record["PubmedData"]["ArticleIdList"]:
                                    ...
                                    if bool(re.search("[/]", str(r))):
                                        ...
                                        if order == len(authors):
                                            ...
                                            pmid[
                                                int(record["MedlineCitation"]["PMID"])
                                            ] = order
                                            dois[str(r)] = order
        ...
        print(
            "Found %s pmids for author %s (for NeuroSynth 3000 database)."
            % (len(pmid), author)
        )
        print(
            "Found %s dois for author %s (for NeuroSynth 525 database)."
            % (len(dois), author)
        )
        return (dois, pmid)
